...
uint8_t rgb_matix_underglow_hue = 1;
...
void update_underglow_hue(bool increase) {
    rgb_matix_underglow_hue += increase ? RGB_MATRIX_HUE_STEP : (-RGB_MATRIX_HUE_STEP);
}
...
bool process_record_user_rgb_matrix(uint16_t keycode, keyrecord_t* record) {
    ...
    switch (keycode) {
        ...
        case RGB_UHI:
            if (record->event.pressed) {
                update_underglow_hue(true);
            }
            break;
        case RGB_UHD:
            if (record->event.pressed) {
                update_underglow_hue(false);
            }
            break;
    }
    return true;
